...
from .formats import plural, human_join # pylint: disable=relative-beyond-top-level
...
class UserFriendlyTime(commands.Converter):
    ...
    async def check_constraints(self, ctx, now, remaining):
        if self.dt < now: # pylint: disable=no-member
            raise commands.BadArgument('This time is in the past.')
    ...
    async def convert(self, ctx, argument):
        ...
        try:
            ...
            dt, status, begin, end, dt_string = elements[0]  # pylint: disable=unused-variable
